...
if __name__ == '__main__':
    ...
    dropout_dpct = 0. # NOTE consistency model doesn't work with dropout
    ...
    N_initial = 2 # 10  # starting value of N in the step schedule (denoted as s_0 in appendix C) - starting from a low value yeilds a decreasing loss curve, while starting from a high value yeilds almost flat loss curve
    N_final =  150 # 1023 # final value of N in the step schedule (denoted as s_1 in appendix C)
    ...
    num_diffusion_processes = int( (N_final - N_initial) * 0.6 ) # 0.6 # smaller jumps between N_k values = higher sample quality (no effect on loss though)
    num_train_steps_per_diffusion_process = int(total_train_steps / num_diffusion_processes) 
    ...
    random_seed = 10
    sample_freq = int(total_train_steps / 40)
    model_save_freq = int(total_train_steps / 4)
    ...
    save_folder = './generated_muse_fsq_transformer_ictct_customDecoder_cfg_mnist' + hyperparam_str
    ...
    muse_ckpt_path = './ckpts/muse_fsq_transformer_ictct_customDecoder_cfg_mnist' + hyperparam_str + '.pt'
    ...
    while epoch < num_epochs:
        ...
        for imgs, labels in pbar2:
            ...
            train_step += 1
            pbar2.update(1)
            pbar2.set_description('N_k:{} loss:{:.10f}'.format(N_k, ema_losses[-1]))

            # save ckpt 
            if train_step % model_save_freq == 0:
                save_ckpt(device, muse_ckpt_path, ema_net, optimizer)
            ...
            if train_step % sample_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    save_image(grid, f"{save_folder}/trainStep={train_step}_Nk={N_k}_caption={sample_caption_string}.png")
